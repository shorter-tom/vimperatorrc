...
let self = liberator.plugins.smooziee = (function(){
  ...
  */
  mappings.addUserMap(
      [modes.NORMAL],
      ["<C-d>"],
      "Smooth scroll down 50%",
      function(count){
        self.smoothScrollBy( Math.floor( window.innerHeight / 2) * (count || 1) );
      },
      { count: true }
  );
  mappings.addUserMap(
  ...
  var PUBLICS = {
    smoothScrollBy: function(moment) {
      ...
      interval = window.eval(liberator.globalVariables.smooziee_scroll_interval || '30');
